...
# ----------------------------------------
class SGN(nn.Module):
    def __init__(self, in_channels=3, out_channels=31, start_channels=64, pad='zero', activ='lrelu', norm='ln', ):
    ...
    def forward(self, x, layers=[], encode_only=False):
        ...
        if len(layers) > 0:
            if encode_only:
                ...
                return [x[:, :, :h_inp, :w_inp]]  # return intermediate features alone; stop in the last layers
            else:
                return x[:, :, :h_inp, :w_inp], [x[:, :, :h_inp, :w_inp]]
        else:
