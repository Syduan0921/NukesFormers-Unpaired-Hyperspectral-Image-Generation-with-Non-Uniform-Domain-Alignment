...
from .KAN_Ours import NukesFormers
from .MST_Plus_Plus import trunc_normal_, MSAB, MST
from .Restormer import Restormer
from .hdnet import SDL_attention, EFF, default_conv, ResBlock_HD
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
             init_gain=0.02, no_antialias=False, no_antialias_up=False, gpu_ids=[], opt=None, stage=3):
    """Create a generator
    ...
    if netG == 'resnet_9blocks':
        ...
        net = MST_(input_nc, output_nc, output_nc, stage=2, num_blocks=[4, 7, 5])
    elif netG == 'NukesFormers':
        net = NukesFormers(input_nc, output_nc, output_nc, stage=stage)
    elif netG == 'hrnet':
        net = SGN(input_nc, output_nc, start_channels=64 if input_nc>3 else 4)
    elif netG == 'AWAN':
        net = AWAN(input_nc, output_nc, channels=200, n_DRBs=8)
    elif netG == 'HSCNN_Plus':
        net = HSCNN_Plus(input_nc, output_nc, num_blocks=20 if input_nc>3 else 3)
    elif netG == 'mst_plus_plus':
        net = MST_Plus_Plus(input_nc, output_nc, output_nc, stage=stage)
    elif netG == 'transformerlayer':
        net = TransformerLayer(input_nc, output_nc, output_nc, stage=stage)
    elif netG == 'restormer':
        net = Restormer(inp_channels=input_nc, out_channels=output_nc, dim=32 if input_nc>3 else 4)
    elif netG == 'hdnet':
        net = HDNet(input_nc, output_nc, n_feats=48 if input_nc>3 else 16)
        ...
        net = nn.Conv2d(input_nc, output_nc, kernel_size=1, stride=1, bias=False)
    elif netG == 'embed_mlp':
        net = MlpEmbedding(input_nc, output_nc, ratio=2, bias=False)
    elif netG == 'feedforward':
        net = FeadForward(input_nc, output_nc, ratio=4, bias=False)
    else:
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        net = NLayerDiscriminator(input_nc, ndf, n_layers=1, norm_layer=norm_layer, no_antialias=no_antialias,)
...
class PatchSampleF(nn.Module):
    ...
    def create_mlp_raw(self, feats):
        input_nc = feats.shape[1]
    ...
    def forward_raw(self, feats, num_patches=64, patch_ids=None):
        return_ids = []
        return_feats = []
        if self.use_mlp and not self.mlp_init:
            self.create_mlp(feats)
        B, H, W = feats.shape[0], feats.shape[2], feats.shape[3]
        feat_reshape = feats.permute(0, 2, 3, 1).flatten(1, 2)
        if num_patches > 0:
...
class FeadForward(nn.Module):
    def __init__(self, input_nc, output_nc, ratio, bias):
        super(FeadForward, self).__init__()
        media_nc = input_nc if input_nc > output_nc else output_nc
        self.net = nn.Sequential(nn.Conv2d(input_nc, media_nc, 1, 1, bias=bias),
                                 nn.Conv2d(media_nc, media_nc, 3, 1, 1, bias=bias),
                                 nn.Conv2d(media_nc, output_nc, 1, 1, bias=bias),
                                 )
        self.skip = nn.Conv2d(input_nc, output_nc, 1, 1, bias=bias)
...
class MlpEmbedding(nn.Module):
    def __init__(self, input_nc, output_nc, ratio, bias):
        ...
        self.layers = nn.Sequential(
            nn.Linear(in_features=input_nc, out_features=media_nc, bias=bias),
            nn.Linear(in_features=media_nc, out_features=media_nc, bias=bias),
            nn.Linear(in_features=media_nc, out_features=output_nc, bias=bias),
        )
        self.skip = nn.Conv2d(input_nc, output_nc, 1, 1, bias=bias)
    def forward(self, x):
        B, C, H, W = x.shape
        ...
        x = self.layers(x)
        x = torch.permute(x, [0, 3, 1, 2]) + x_skip
        return x




class MST_(nn.Module):
    def __init__(self, in_dim=31, out_dim=31, dim=31, stage=2, num_blocks=[1,1,1]):
        super(MST_, self).__init__()
        self.dim = dim
        self.stage = stage
...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3, out_channels=31, n_feat=31, stage=1):
        super(MST_Plus_Plus, self).__init__()
        self.stage = stage
        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2, bias=False)
        self.body = nn.ModuleList([MST(in_dim=in_channels, out_dim=out_channels, dim=out_channels, stage=2, num_blocks=[2, 4, 4]) for _ in range(stage)])
...
class ResnetGenerator(nn.Module):
    ...
    def mask_preprocess(self, input, mask):
        inputA = input * mask
        inputB = input - inputA
        return inputA, inputB

    def feat_process(self, x, y, mask):
        x = x * mask
        y = y * (1 - mask)
        feat = x + y
        return feat
    def forward(self, input, layers=[], encode_only=False):
        ...
        if len(layers) > 0:
            ...
            return fake
    def forward_old(self, input, mask, layers=[], encode_only=False):
        if -1 in layers:
            layers.append(len(self.model))
        inputA, inputB = self.mask_preprocess(input, mask)
        if len(layers) > 0:
            featA, featB = inputA, inputB
            feat = input
            feats = []
            for layer_id, layer in enumerate(self.model):
                # print(layer_id, layer)
                featA, featB = layer(featA), layer(featB)
                feat = featA + featB
                if layer_id in layers:
                    # print("%d: adding the output of %s %d" % (layer_id, layer.__class__.__name__, feat.size(1)))
                    feats.append(feat)
            ...
            """Standard forward"""
            fakeA, fakeB = self.model(inputA), self.model(inputB)
            fake = self.feat_process(fakeA, fakeB, mask)
            return fake
...
class HDNet(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=31, conv=default_conv, n_feats=32):
    ...
    def forward(self, x, layers=[], encode_only=False):
        ...
        if len(layers) > 0:
            if encode_only:
                # print('encoder only return features')
                return [x] # return intermediate features alone; stop in the last layers
            else:
                return x, [x]
        else:
            return x
...
class TransformerLayer(nn.Module):
    def __init__(self, in_channels=3, out_channels=31, n_feat=31, stage=3):
        ...
        self.stage = stage
        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2, bias=False)
        self.body = TransformerBlock(in_dim=in_channels, out_dim=out_channels, dim=out_channels, stage=2, num_blocks=[1, 1, 1])
        self.conv_out = nn.Conv2d(n_feat, out_channels, kernel_size=3, padding=(3 - 1) // 2, bias=False)
    ...
    def forward(self, x, layers=[], encode_only=False):
        ...
        if len(layers) > 0:
            feats = []
            if encode_only:
                feats = self.body(h, layers, encode_only)
                # print('encoder only return features')
                return feats  # return intermediate features alone; stop in the last layers
            else:
                h, feats = self.body(h, layers, encode_only)
                h = self.conv_out(h)
                h += x
                return h[:, :, :h_inp, :w_inp], feats
        else:
            h = self.body(h, layers, encode_only)
            h = self.conv_out(h)
            h += x
            return h[:, :, :h_inp, :w_inp]
