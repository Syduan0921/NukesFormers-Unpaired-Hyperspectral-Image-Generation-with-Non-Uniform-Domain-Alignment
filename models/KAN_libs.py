...
class KANLinear(torch.nn.Module):
    def __init__(
            ...
            use_nurbs=False,  # Determine whether use the NURBS
    ):
        ...
        if self.use_nurbs:
            self.control_point_weights = torch.nn.Parameter(
    ...
    def construct_spline_base_matrix(self, grid: torch.Tensor) -> torch.Tensor:
        base_matrix = torch.eye(self.grid_size + self.spline_order, device=grid.device)

        for k in range(1, self.spline_order + 1):
            new_matrix = torch.zeros_like(base_matrix)
            for i in range(self.grid_size + self.spline_order - k):
                new_matrix[i] = ((grid[:, i + 1] - grid[:, i]) * base_matrix[i] + (
                        grid[:, i + k + 1] - grid[:, i + 1]) * base_matrix[i + 1]) / \
                                (grid[:, i + k + 1] - grid[:, i])
            base_matrix = new_matrix
        return base_matrix

    def b_splines_matrix(self, x: torch.Tensor):
        assert x.dim() == 2 and x.size(1) == self.in_features
        grid: torch.Tensor = self.grid
        grid_min, grid_max = grid.min(), grid.max()
        x = x.unsqueeze(-1)

        # 1. Create knot vector based on grid and spline_order
        knot_vector = torch.cat([
            torch.full((self.spline_order,), grid_min, device=x.device),  # Pad with the minimum value for spline order
            grid[0],  # Actual grid values
            torch.full((self.spline_order,), grid_max, device=x.device)  # Pad with the maximum value for spline order
        ], dim=0)  # (grid_size + 2 * spline_order,)

        # 2. Initialize B-spline basis matrix (Identity matrix represents 0th-degree B-splines)
        B = torch.zeros(self.grid_size + self.spline_order, self.grid_size + self.spline_order, device=x.device)
        for i in range(self.grid_size + self.spline_order):
            B[i, i] = 1  # Initialize identity matrix for 0-degree splines

        # 3. Apply De Boor's algorithm using matrix operations to calculate higher-degree splines
        for k in range(1, self.spline_order + 1):
            left_term = (x - knot_vector[:-k]) / (
                        knot_vector[k:] - knot_vector[:-k])  # Left term in de Boor's recurrence
            right_term = (knot_vector[k + 1:] - x) / (knot_vector[k + 1:] - knot_vector[1:-k])  # Right term

            # Update B-spline basis matrix
            B[:-1, :-1] = left_term * B[:-1, :-1] + right_term * B[1:, 1:]

        bases = B.expand(self.in_features, -1, -1)  # Expand to match input size

        if self.use_nurbs:
            # Apply NURBS weighting if enabled
            weights = self.control_point_weights.unsqueeze(0)  # (1, out, in)
            weights = weights.permute(0, 2, 1).unsqueeze(2).expand(bases.size(0), -1, bases.size(2),
                                                                   -1)  # (B, in, grid+order, out)
            weighted_bases = bases.unsqueeze(-1) * weights  # (B, in, grid+order, out)

            # Normalization, Sum to one
            normalization_factor = weighted_bases.sum(dim=-1, keepdim=True) + 1e-4  # (B, in, grid+order, 1)
            bases = (weighted_bases / normalization_factor).sum(dim=-1)  # (B, in, grid+order)
    ...
    def b_splines(self, x: torch.Tensor):
        ...
        if self.use_nurbs:
            weights = self.control_point_weights.unsqueeze(0)  # (1, out, in)
            weights = weights.permute(0, 2, 1).unsqueeze(2).expand(bases.size(0), -1, bases.size(2),
                                                                   -1)  # (B, in, gird+order, out)
            weighted_bases = bases.unsqueeze(-1) * weights  # (B, in, gird+order, out)

            # weighted_sum = weighted_bases.sum(dim=-1) #(B, in, gird+order)
            # Normalization, Sum to one
            normalization_factor = weighted_bases.sum(dim=-1, keepdim=True) + 1e-4  # (B, in, gird+order, 1)
            bases = (weighted_bases / normalization_factor).sum(dim=-1)  # (B, in, gird+order)
...
class BSplineFunction(nn.Module):
    def __init__(self, grid_min: float = -2.,
        ...
        self.knots = torch.linspace(grid_min, grid_max, num_basis + degree + 1).cuda()  # Uniform knots
...
def runs():
    data = torch.randn([3, 64]).cuda()
    # model = FastKANConvLayer(3, 10, kernel_size=3, padding=(3 - 1) // 2,
    # bias=False, stride=1, kan_type='BSpline', use_base_update=False).cuda()
    model = KANLinear(in_features=64, out_features=128).cuda()
    x1 = model.b_splines(data)
    x2 = model.b_splines_matrix(data)
    flag = 1
