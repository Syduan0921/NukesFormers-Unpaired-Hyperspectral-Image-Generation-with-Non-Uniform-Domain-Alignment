...
class NukesFormers(nn.Module):
    def __init__(self, in_channels=3, out_channels=31, n_feat=31, stage=3):
        super(NukesFormers, self).__init__()
        self.stage = stage
        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2,bias=False)
        self.body = nn.ModuleList(
            [Nukes(in_dim=n_feat, out_dim=n_feat, dim=n_feat, stage=2, num_blocks=[2, 4, 4]) for _ in
             range(stage)])
